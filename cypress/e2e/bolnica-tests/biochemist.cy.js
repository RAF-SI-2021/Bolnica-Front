...
describe('Technician', () => {
    ...
    beforeEach(() => {
        ...
        cy.visit('http://localhost:3000/biochemist');
    })

    it('should be able to see unprocessed work orders/reports for today and their details', () => {  
        cy.get('.user-name').should('be.visible')
                .should('contain', 'Jasda');//localStorage.getItem('name'));
        cy.get('.user-title').should('be.visible')
                .should('contain', 'Alskcna');//localStorage.getItem('title'));
    ...
    it('should be able to see user profile and update its data', () => {//vraca na admina 
        ...
        cy.get('.nav-item:nth-child(2) > .nav-link > .familyFix').should('be.visible').should('contain', 'Profil').click({ multiple: true });
