...
describe('Nurse', () => {
    ...
    beforeEach(() => {
        ...
        cy.visit('http://localhost:3000/recepcionist');
    })

    it('should be able to add new patients to system', () => {
        cy.get('ul').should('be.visible');
        cy.get('ul > li:nth-child(2)').should('be.visible').should('contain', 'Dodavanje pacijenta').click();
        ...
        cy.get('input[name="imeRoditelja"]').should('be.visible').type(chance.name());
        cy.get('input[name="jmbg"]').should('be.visible').type(chance.integer({ min: 1000000000000, max: 9999999999999 }));
        cy.get('.form-group-custom:nth-child(5) > .margin-right:nth-child(1)').should('be.visible').click().type('1980-12-12');
        cy.get('input[name="mestoRodjenja"]').should('be.visible').type(chance.city());
        cy.get('input[name="adresa"]').should('be.visible').type(chance.address());
        cy.get('input[name="mestoStanovanja"]').should('be.visible').type(chance.city());
        cy.get('input[name="zemljaStanovanja"]').should('be.visible').type(chance.country());
        cy.get('input[name="zemljaDrzavljanstva"]').should('be.visible').type(chance.country());
        cy.get('input[name="kontaktTelefon"]').should('be.visible').type(chance.phone());
        cy.get('input[name="email"]').should('be.visible').type("testtestic@gmail.com");
        cy.get('input[name="imeStaratelj"]').should('be.visible').type(chance.name());
        cy.get('input[name="jmbgStaratelj"]').should('be.visible').type(chance.integer({ min: 1000000000000, max: 9999999999999 }));
        cy.get('select[name="porodicniStatus"]').should('be.visible').select(2);
        cy.get('select[name="bracniStatus"]').should('be.visible').select(3);
        cy.get('input[name="brojDece"]').should('be.visible').type(chance.integer({ min: 1, max: 5 }));
        cy.get('input[name="zanimanje"]').should('be.visible').type(chance.profession());
        ...
        cy.get('button').should('be.visible').should('contain', 'Registruj pacijenta').last().click();
    ...
    it('should be able to schedule/view patients for a specific doctor in specific department in calendar', () => { //jebada
        cy.get('ul').should('be.visible');
        cy.get('ul > li:nth-child(3)').should('be.visible').should('contain', 'Zakazivanje pregleda').click({ multiple: true });
        cy.url({ timeout: 10000 }).should('contain', '/recepcionist/add-appointment');
        ...
        cy.get('ul > li:nth-child(1)').should('be.visible').should('contain', 'Početna').click({ multiple: true });
    })
    
    it('should be able to see list of all patients', () => {//meni ne radi
        cy.get('ul').should('be.visible');
        cy.get('ul > li:nth-child(4)').should('be.visible').should('contain', 'Pretraga pacijenata').click({ multiple: true });
        cy.url({ timeout: 10000 }).should('contain', '/recepcionist/search-patient');
