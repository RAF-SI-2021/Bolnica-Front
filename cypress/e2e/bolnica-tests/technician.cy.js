...
describe("Technician", () => {
  ...
  it("should be able to schedule patient for labaratory visit", () => {
    //meni ne radi nzm
    ...
    cy.get(
      ".form-group-custom:nth-child(3) > .margin-right:nth-child(1)"
    ).click();
    cy.get(".form-group-custom:nth-child(3) > .margin-right:nth-child(1)").type(
      "2023-07-01"
    );
  ...
  it("should be able to see schedueled patients by their LBP, date or both", () => {
    cy.get(".flex:nth-child(1)").click();
    cy.url({ timeout: 5000 }).should("contain", "/technician/visits");
    cy.get(".nav-item > .disabled")
      .should("contain", "Pregled zakazanih poseta")
      .click({ multiple: true });
    cy.get("form").should("be.visible");
    cy.get('input[name="lbp"]').should("be.visible").type(lbp);
    cy.get(".form-group-custom > .form-group-custom > .margin-right").click({
      multiple: true,
    });
    cy.get(".form-group-custom > .form-group-custom > .margin-right").type(
      "2023-07-01"
    );
    ...
    cy.get(".form-group-custom:nth-child(3) > .buttonForm").click({
      multiple: true,
    });
  });
  ...
  //   ).should("contain", lbp);
  //   cy.get(
  //     ".myTable > .familyFix > tr:nth-child(1) > td > .d-flex > .buttonKreirajNalog"
  //   )
