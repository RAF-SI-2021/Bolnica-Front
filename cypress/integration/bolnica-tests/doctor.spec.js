...
describe('Doctor', () => {
    ...
    it('should containt all homepage informations after login', () => {
        ...
        cy.get('.flex:nth-child(1) > .statistics > .number').should('be.visible')
                    .should('contain', '34');//localStorage.getItem('numberOfAppointments'));
        cy.get('.flex:nth-child(2) > .statistics').should('be.visible');
        cy.get('.flex:nth-child(2) > .statistics > .text').should('be.visible')
                    .should('contain', 'Broj pacijenata');
        cy.get('.flex:nth-child(2) > .statistics > .number').should('be.visible')
                    .should('contain', '10');//localStorage.getItem('numberOfPatients'));
        cy.get('.flex:nth-child(3) > .statistics').should('be.visible');
        cy.get('.flex:nth-child(3) > .statistics > .text').should('be.visible')
                    .should('contain', 'Operacije');
        cy.get('.flex:nth-child(3) > .statistics > .number').should('be.visible')
    ...
    it('should be able to see appointments', () => {
        cy.get('ul').should('be.visible');
        cy.get('ul > li:nth-child(3)').should('be.visible').should('contain', 'Zakazani pregledi').click({ multiple: true });
    ...
    it('should be able to write a refferal for labaratory', () => {
        cy.get('ul').should('be.visible');
        cy.get('ul > li:nth-child(4)').should('be.visible').should('contain', 'Kreiraj uput').click({ multiple: true });
        ...
        cy.get('input[name="comment"]').should('be.visible').clear().type(chance.sentence({ words: 5 }));
    ...
    it('should be able to write a refferal for diagnosis', () => {
        ...
        cy.get('input[name="comment"]').should('be.visible').clear().type(chance.sentence({ words: 3 }));
        cy.get('select[name="diagnosis"]').should('be.visible').select(5);  
        cy.get('input[name="reason"]').should('be.visible').clear().type(chance.sentence({ words: 5 }));
        cy.get('button').should('be.visible').last().click({ multiple: true });
    })
